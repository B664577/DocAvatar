...
# ==================== Configuration ====================
DEMO_CONFIG_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "demo_config.json"))
...
DEFAULT_CONFIG = {
    ...
    # Online (StepFun) defaults
    'inference_mode': 'vllm',
    'online_vendor': 'StepFun',
    'online_model': 'step-1o-turbo-vision',
    'online_api_key': "",
    'online_base_url': "https://api.stepfun.com/v1",
    'online_system_prompt': "你是由阶跃星辰提供的AI聊天助手，你擅长中文，英文，以及多种其他语言的对话。在保证用户数据安全的前提下，你能对用户的问题和请求，作出快速和精准的回答。同时，你的回答和建议应该拒绝黄赌毒，暴力恐怖主义的内容",
    'online_cache': True,
}
...
def save_persisted_config(cfg: dict):
    try:
        ...
        print(f"Failed to save persisted config: {e}")

# ==================== Global Variables ====================
# Store current configuration (merge persisted)
...
# Create DotsOCRParser instances for different modes
parser_instances = {}

def get_parser(mode="vllm", **kwargs):
    """获取指定模式的解析器实例"""
    global parser_instances
    
    # 创建参数字典
    parser_kwargs = {
        'ip': kwargs.get('ip', DEFAULT_CONFIG['ip']),
        'port': kwargs.get('port', DEFAULT_CONFIG['port_vllm']),
        'dpi': 200,
        'min_pixels': kwargs.get('min_pixels', DEFAULT_CONFIG['min_pixels']),
        'max_pixels': kwargs.get('max_pixels', DEFAULT_CONFIG['max_pixels'])
    }
    
    if mode == "hf":
        parser_kwargs['use_hf'] = True
    elif mode == "cpu":
        parser_kwargs['use_hf'] = True  # CPU模式使用HuggingFace后端但强制使用CPU
    elif mode == "online":
        parser_kwargs['use_online'] = True
        parser_kwargs['online_vendor'] = kwargs.get('online_vendor', current_config.get('online_vendor'))
        parser_kwargs['online_model'] = kwargs.get('online_model', current_config.get('online_model'))
        parser_kwargs['online_api_key'] = kwargs.get('online_api_key', current_config.get('online_api_key'))
        parser_kwargs['online_base_url'] = kwargs.get('online_base_url', current_config.get('online_base_url'))
        parser_kwargs['online_system_prompt'] = kwargs.get('online_system_prompt', current_config.get('online_system_prompt'))
    else:  # vllm mode
        parser_kwargs['use_hf'] = False
    
    # 创建解析器实例
    key = f"{mode}_{hash(str(sorted(parser_kwargs.items())))}"
    if key not in parser_instances:
        parser_instances[key] = DotsOCRParser(**parser_kwargs)
    
    return parser_instances[key]

# Default parser (vllm mode)
dots_parser = get_parser("vllm")
...
def process_image_inference(session_state, test_image_input, file_input,
                          prompt_mode, inference_mode, server_ip, server_port, min_pixels, max_pixels,
                          fitz_preprocess=False,
                          online_vendor=None, online_model=None, online_api_key=None, online_base_url=None, online_system_prompt=None, online_cache=True
                          ):
    ...
    if inference_mode == 'online':
        current_config.update({
            ...
            'online_cache': bool(online_cache),
        })
    ...
    try:
        ...
        return None, f"⚠️ Failed to initialize {inference_mode} parser: {str(e)}", "", "", gr.update(value=None), None, "", session_state
    ...
    try:
        if file_ext == '.pdf':
            ...
            pdf_result = parse_pdf_with_high_level_api(parser, input_file_path, prompt_mode)
            ...
            if inference_mode == 'online' and bool(online_cache):
                save_persisted_config(current_config)
            return (
            ...
            parse_result = parse_image_with_high_level_api(parser, image, prompt_mode, fitz_preprocess)
    ...
    with gr.Blocks(theme="ocean", css=css, title='dots.ocr') as demo:
            ...
            with gr.Column(scale=1, elem_id="left-panel"):
                ...
                with gr.Accordion("🛠️ Advanced Configuration", open=False):
                    ...
                    inference_mode = gr.Radio(
                        choices=["vllm", "hf", "cpu", "online"],
                        value=current_config.get('inference_mode', 'vllm'),
                        label="🚀 Inference Mode",
                        info="Choose inference backend: vLLM (fast), HuggingFace (compatible), CPU, or Online API"
                    )
                    ...
                    with gr.Row():
                        server_ip = gr.Textbox(label="Server IP", value=current_config.get('ip', DEFAULT_CONFIG['ip']))
                        server_port = gr.Number(label="Port", value=current_config.get('port_vllm', DEFAULT_CONFIG['port_vllm']), precision=0)
                    with gr.Row():
                        min_pixels = gr.Number(label="Min Pixels", value=current_config.get('min_pixels', DEFAULT_CONFIG['min_pixels']), precision=0)
                        max_pixels = gr.Number(label="Max Pixels", value=current_config.get('max_pixels', DEFAULT_CONFIG['max_pixels']), precision=0)

                    gr.Markdown("### 🌐 Online Provider Configuration")
                    with gr.Row():
                        online_vendor = gr.Dropdown(
                            label="Vendor",
                            choices=["StepFun"],
                            value=current_config.get('online_vendor', 'StepFun'),
                        )
                        online_model = gr.Dropdown(
                            label="Model",
                            choices=["step-1o-turbo-vision", "step-1-8k"],
                            value=current_config.get('online_model', 'step-1o-turbo-vision'),
                        )
                    with gr.Row():
                        online_api_key = gr.Textbox(label="API Key", value=current_config.get('online_api_key', ''), type="password")
                        online_base_url = gr.Textbox(label="Base URL", value=current_config.get('online_base_url', 'https://api.stepfun.com/v1'))
                    online_system_prompt = gr.Textbox(
                        label="System Prompt",
                        value=current_config.get('online_system_prompt', DEFAULT_CONFIG['online_system_prompt']),
                        lines=4,
                        max_lines=8,
                    )
                    online_cache = gr.Checkbox(label="Cache online config", value=bool(current_config.get('online_cache', True)))
        ...
        process_btn.click(
            ...
            inputs=[
                ...
                prompt_mode, inference_mode, server_ip, server_port, min_pixels, max_pixels, 
                fitz_preprocess,
                online_vendor, online_model, online_api_key, online_base_url, online_system_prompt, online_cache
            ],
            outputs=[
